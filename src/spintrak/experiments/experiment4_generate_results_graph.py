...
def main(
    ...
    output_filename: str = "experiment_4_results.svg",
):
    """
    Generate plot with results of the experiment 4.
    """
    ...
    replace_dict = {
        "Slow, melancholic salsa groove": "Sad salsa",
        "Fast, happy salsa groove": "Happy salsa",
        "Slow, melancholic indie rock riff": "Sad rock",
        "Fast, happy indie rock riff": "Happy rock",
    }
    df.replace(
    ...
    order = ["Sad salsa", "Happy salsa", "Sad rock", "Happy rock", "Dysrhytmic techno"]
    df = df.sort_values(
        by="Generated sample", key=lambda x: x.map(lambda y: order.index(y))
    )
    (
        so.Plot(
            df,
