...
class LMModel(StreamingModule):
    ...
    def forward(self, sequence: torch.Tensor,
        ...
        if condition_tensors is None:
            ...
            condition_tensors = self.condition_provider(tokenized).detach()
        else:
            for key, value in condition_tensors.items():
                condition_tensors[key] = (value[0].detach(), value[1])
            assert not conditions, "Shouldn't pass both conditions and condition_tensors."

        self.transformer.detach_streaming()
        input_, cross_attention_input = self.fuser(input_, condition_tensors)

        
        out = self.transformer(input_, cross_attention_src=cross_attention_input,
                               src_mask=(self.attn_mask_per_stage[stage].detach() if stage >= 0 else None))
        if self.out_norm:
    ...
    def _sample_next_token(self,
                           ...
                           save_grads: bool = False) -> torch.Tensor:
        ...
        if save_grads:
            ...
            if not self.gradients:
                self.gradients = [param.grad for param in self.parameters() if param.requires_grad and param.grad is not None]
            else:
                new_grads = [param for param in self.parameters() if param.requires_grad and param.grad is not None]
                self.gradients = [g + ng for g, ng in zip(self.gradients, new_grads)]
        ...
        with self.streaming():
            ...
            for offset in range(start_offset_sequence, gen_sequence_len):
                ...
                next_token = self._sample_next_token(
                    ...
                    cfg_coef=cfg_coef, two_step_cfg=two_step_cfg, save_grads=save_grads)
